...
export const EditarOLT = () => {
  ...
  useEffect(() => {
    ...
    const obtenerDatosISP = async () => {
      try {
        const respuesta = await fetch(
          `http://localhost:3001/obtenerISP/${id}`,
          {
            credentials: "include",
          }
        );
        if (!respuesta.ok) {
          throw new Error("No se pudieron obtener los datos del ISP");
        } 
        const datosISP = await respuesta.json();
          // Aquí deberías actualizar el estado del formulario con los datos obtenidos
          // Asegúrate de adaptar esta parte según la estructura de tu respuesta
          setFormData({
            nombreOLT: datosISP.nombre_olt,
            marcaOLT: datosISP.marca_olt,
            modeloOLT: datosISP.modelo_olt,
            accesos: {
              ssh: {
                ip: datosISP.ip_ssh.toString(),
                usuario: datosISP.usuario_ssh,
                contraseña: "", // La contraseña probablemente no se envíe por razones de seguridad
                puerto: datosISP.puerto_ssh.toString(),
              },
              telnet: {
                ip: datosISP.ip_telnet.toString(),
                usuario: datosISP.usuario_telnet,
                contraseña: "",
                puerto: datosISP.puerto_telnet.toString(),
              },
              snmp: {
                ip: datosISP.ip_snmp.toString(),
                usuario: datosISP.usuario_snmp,
                contraseña: "",
                puerto: datosISP.puerto_snmp.toString(),
                comunidadLectura: datosISP.comunidad_lectura_snmp,
                comunidadEscritura: datosISP.comunidad_escritura_snmp,
              },
            },
          });

      } catch (error) {
        toast.error("Error al cargar los datos del ISP.", {
          position: "top-right",
          autoClose: 5000,
          closeOnClick: true,
          pauseOnHover: true,
  ...
  }, [formData.marcaOLT]); // Este useEffect ahora solo se activará cuando `formData.marcaOLT` cambie.
  ...
  const validarFormulario = () => {
    ...
    // Marca OLT
    if (!formData.marcaOLT.toString().trim()) {
      nuevosErrores.marcaOLT =
        "La seleccion de un modelo de OLT es obligatoria.";
    }

    // Modelo OLT
    if (!formData.modeloOLT.toString().trim()) {
    ...
    if (!formData.accesos.ssh.usuario.trim()) {
      nuevosErrores.usuarioSSH =
        "El nombre de usuario de SSH es obligatorio.";
    }
  ...
  const handleSubmit = async (e) => {
    ...
    // Asigna un valor por defecto a las contraseñas si están vacías
    ["ssh", "telnet", "snmp"].forEach((acceso) => {
      if (!datosParaEnviar.accesos[acceso].contraseña) {
        datosParaEnviar.accesos[acceso].contraseña = ""; // Asigna una cadena vacía en lugar de eliminar
      }
    });

    try {
      const respuesta = await fetch(`http://localhost:3001/editarISP/${id}`, {
        method: "PATCH",
        credentials: "include",
        headers: {
      ...
      if (respuesta.ok) {
        toast.success(`OLT editado exitosamente.`, {
          position: "top-right",
          autoClose: 1850,
          closeOnClick: true,
          pauseOnHover: true,
