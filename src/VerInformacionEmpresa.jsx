...
export const VerInformacionEmpresa = () => {
  ...
  const [logoSubidoDesdeBD, setLogoSubidoDesdeBD] = useState(false);
  ...
  useEffect(() => {
    const cargarInfoEmpresaUsuario = async () => {
      try {
        const respuesta = await fetch('http://localhost:3001/obtenerInfoEmpresaUsuario', {
          credentials: 'include', // Asegúrate de incluir credenciales para sesiones
        });
        ...
        const datosEmpresa = await respuesta.json();

        // Dividir el nombre del proyecto desde el punto si existe
        let nombreProyecto = datosEmpresa.nombre_proyecto || '';
        let dominioSeleccionado = '';

        // Verifica si el nombre del proyecto contiene un punto para dividirlo
        if (nombreProyecto.includes('.')) {
          const partes = nombreProyecto.split('.');
          nombreProyecto = partes[0]; // La primera parte antes del punto
          dominioSeleccionado = '.' + partes.slice(1).join('.'); // El resto después del punto, agregando el punto al inicio
        }

        // Actualiza el estado del formulario con los datos de la empresa
        setFormData({
          ...formData,
          pais: datosEmpresa.pais_cod || '',
          nombreComercial: datosEmpresa.nombre_comercial || '',
          identificacionFiscal: datosEmpresa.identificacion_fiscal || '',
          direccion: datosEmpresa.direccion || '',
          nombreContacto: datosEmpresa.nombre_contacto || '',
          telefonoMovil: datosEmpresa.telefono1 || '',
          telefono: datosEmpresa.telefono2 ? datosEmpresa.telefono2.toString() : '' || '',
          nombreProyecto: nombreProyecto,
          dominioSeleccionado: dominioSeleccionado,
          correoFacturacion: datosEmpresa.correo_facturacion || '',
          nombreServidorCorreo: datosEmpresa.nombre_servidor_correo || '',
          puertoSmtp: datosEmpresa.puerto_smtp ? datosEmpresa.puerto_smtp.toString() : '' || '',
          contrasenaCorreo: '', // Por seguridad, no devuelvas las contraseñas
          confirmacionContrasenaCorreo: '',
          archivoFirmaElectronica: datosEmpresa.archivo_firma_electronica,
          contrasenaFirmaElectronica: '',
          confirmacionContrasenaFirmaElectronica: ''
        });

        if (datosEmpresa.archivo_firma_electronica) {
          setArchivoSubidoDesdeBD(true);
        }

        if (datosEmpresa.logo) {
          setLogoSubidoDesdeBD(true);
        }

        // Verificar si alguno de los campos relacionados con el correo de facturación tiene valor
        const tieneDatosCorreo = datosEmpresa.correo_facturacion || datosEmpresa.nombre_servidor_correo || datosEmpresa.puerto_smtp || datosEmpresa.contrasena_correo;
        setMostrarCamposCorreo(!!tieneDatosCorreo);
  ...
  // Estado para almacenar los paises recibidos
  ...
  const [archivoSubidoDesdeBD, setArchivoSubidoDesdeBD] = useState(false);
  const [dominioSeleccionado, setDominioSeleccionado] = useState(''); //ver si se quita o no
  const [cargando, setCargando] = useState(false);
  const [dominioVerificado, setDominioVerificado] = useState(null);
  const [errores, setErrores] = useState({});
  const phoneUtil = PhoneNumberUtil.getInstance();
  const [eliminarArchivoSubido, setEliminarArchivoSubido] = useState(false);
  ...
  const manejarEliminarArchivo = () => {
    setEliminarArchivoSubido(true);
    setArchivoFirma(null); // Elimina la referencia al archivo seleccionado si existe
    // Aquí deberías ajustar cualquier otra lógica necesaria para tratar el archivo como eliminado
  ...
  const cancelarEliminacionArchivo = () => {
    setEliminarArchivoSubido(false);
    // Asumiendo que tienes alguna manera de restaurar el archivo desde la BD o mantener su referencia
    // Si necesitas volver a cargar el archivo desde la BD, aquí deberías invocar esa lógica
  };
  ...
  const validarFormulario = () => {
    ...
    if (mostrarCamposCorreo) {
      ...
      if (formData.contrasenaCorreo.trim() !== formData.confirmacionContrasenaCorreo.trim()) {
  ...
  const handleSubmit = (e) => {
    ...
    const adjustedData = {
      ...
      eliminarArchivoSubido: eliminarArchivoSubido,
    };
    ...
    const url = 'http://localhost:3001/actualizarEmpresa';
    const fetchOptions = {
      method: 'PATCH',
      credentials: 'include',
      ...
      <div className="md:mt-2 flex items-center justify-center p-4">
        ...
        >
          <div className="flex flex-wrap -mx-3 sm:mb-6">
            <div className="w-full xls:w-1/2 px-3 mb-6 md:mb-0">
              ...
              <select
                ...
                className="mt-2 h-10 bg-gray-50 border border-sky-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 bg-sky-100 placeholder-gray-500 text-dark focus:outline-none"
              >
                <option disabled value="">
                  Seleccione un pais
                </option>
                {paises.map((pais) => (
                  <option key={pais.pais_cod_pa} value={pais.pais_cod_pa}>
                    {pais.pais_des_pa}
                  </option>
                ))}
              </select>
              {errores.pais && (
                <p className="text-red-500 text-xs italic">{errores.pais}</p>
              )}
            </div>
          ...
          <div className='flex flex-wrap -mx-3 sm:mb-6'>
            ...
            <div className='w-full xls:w-1/2 px-3 mb-6 md:mb-0'>
              <label className='block text-sm font-semibold text-gray-900'>Logo:
                {logoSubidoDesdeBD && " (Subido)"}
              </label>
            ...
            <div className="w-full xls:w-1/2 px-3 mb-5 md:mb-0">
              ...
              <div className="relative h-11">
                ...
                <input
                  ...
                  className="mt-2 h-10 bg-gray-50 border border-sky-300 text-gray-900 text-sm rounded-lg block w-full ps-10 p-2.5 bg-sky-100 placeholder-gray-500 text-dark focus:outline-none"
                  placeholder={
                    formData.pais
                      ? "Ingrese un teléfono celular"
                      : "Seleccione un país"
                  }
          ...
          <div className="flex flex-wrap -mx-3 mb-6">
            ...
            <div className="w-full xls:w-1/2 px-3 flex items-end">
              <select
                id="dominios"
                value={formData.dominioSeleccionado}
              ...
              <button
                ...
                className="ml-3 bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded h-11"
                disabled={cargando || isSubmitting}
              >
                {cargando ? <FaSpinner /> : <FaCheck />}
              </button>
            </div>
            {errores.dominio && (
              ...
              <div className='flex flex-wrap mx-6 sm:mx-14 mb-2'>
                ...
                <div className="w-full px-3 mt-6">
                  ...
                  <div className="flex items-center bg-gray-50 border border-sky-300 text-gray-900 text-sm rounded-lg bg-sky-100 text-dark focus:outline-none">
                    <input
                      type={
                        showPassword.contrasenaCorreo ? "text" : "password"
                      }
                    ...
                    <button
                      type="button"
                      onClick={() => toggleShowPassword("contrasenaCorreo")}
                      className="px-3 rounded-r-lg focus:outline-none"
                    >
                      {showPassword.contrasenaCorreo ? (
                        <FaEyeSlash className="text-sky-400 text-sky-400 bg-transparent" />
                      ) : (
                        <FaEye className="text-sky-400 text-sky-400 bg-transparent" />
                      )}
                    </button>
                  </div>

                  {errores.contrasenaCorreo && (
                    <p className="text-red-500 text-xs italic">
                      {errores.contrasenaCorreo}
                    </p>
                  )}
                ...
                <div className="w-full px-3 mt-6">
                  ...
                  <div className="flex items-center bg-gray-50 border border-sky-300 text-gray-900 text-sm rounded-lg bg-sky-100 text-dark focus:outline-none">
                    ...
                    <button
                      type="button"
                      onClick={() =>
                        toggleShowPassword("confirmacionContrasenaCorreo")
                      }
                      className="px-3 rounded-r-lg focus:outline-none"
                    >
                      {showPassword.confirmacionContrasenaCorreo ? (
          ...
          <div className="flex flex-wrap -mx-3 sm:mb-6">
            <div className="w-full px-3 mb-4 md:mb-0">
              <label className="block text-sm font-semibold text-gray-900">
                Archivo firma electronica:{" "}
                <span className="text-sm ml-1 text-gray-500">
                  opcional
                  {archivoSubidoDesdeBD && !eliminarArchivoSubido && " (Subido)"}
                  {eliminarArchivoSubido && " (Eliminado)"}
                  {!eliminarArchivoSubido && archivoSubidoDesdeBD && (
                    <a onClick={manejarEliminarArchivo} className="ml-2 text-blue-600 hover:text-blue-800">Eliminar</a>
                  )}
                  {
                    eliminarArchivoSubido && (
                      <a onClick={cancelarEliminacionArchivo} className="ml-2 text-blue-600 hover:text-blue-800">Cancelar</a>
                    )
                  }
                </span>
              </label>
              <input
                className="mt-2 h-10 block w-full text-xs text-gray-900 border border-sky-300 rounded-lg cursor-pointer bg-gray-50 text-gray-500 focus:outline-none bg-sky-100 p-[9px]"
                type="file"
                accept=".p12"
                onChange={handleArchivoFirmaChange}
              />
              {errores.archivoFirmaElectronica && (
                <p className="text-red-500 text-xs italic">
                  {errores.archivoFirmaElectronica}
                </p>
              )}
          ...
          {(archivoFirma || (formData.archivoFirmaElectronica && !eliminarArchivoSubido)) && (
            <div className="flex flex-wrap -mx-3">
              <div className="w-full xls:w-1/2 px-3 mb-6 md:mb-0">
                ...
                <div className="flex items-center bg-gray-50 border border-sky-300 text-gray-900 text-sm rounded-lg bg-sky-100 text-dark focus:outline-none">
                  ...
                  <button
                    type="button"
                    onClick={() => toggleShowPassword("contrasenaFirma")}
                    className="px-3 rounded-r-lg focus:outline-none"
                  >
                ...
                {errores.contrasenaFirmaElectronica && (
                  <p className="text-red-500 text-xs italic">
                    {errores.contrasenaFirmaElectronica}
                  </p>
                )}
              ...
              <div className="w-full xls:w-1/2 px-3 md:mb-0">
                ...
                <div className="flex items-center bg-gray-50 border border-sky-300 text-gray-900 text-sm rounded-lg bg-sky-100 text-dark focus:outline-none">
                  ...
                  <button
                    type="button"
                    onClick={() =>
                      toggleShowPassword("confirmacionContrasenaFirma")
                    }
                    className="px-3 rounded-r-lg focus:outline-none"
                  >
                    {showPassword.confirmacionContrasenaFirma ? (
          ...
          <button
            type="submit"
            disabled={cargando || isSubmitting}
            className="mt-6 mx-auto w-1/2 sm:w-3/4 md:w-1/2 lg:w-1/3 xl:w-1/4 block text-dark focus:outline-none focus:ring-4 font-medium rounded-full text-sm px-5 py-2.5 text-center bg-green-400 hover:bg-green-500 focus:ring-green-500 -mb-4"
          >
            Guardar
          </button>
        </form>
